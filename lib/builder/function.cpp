    ...
    const std::optional<Field> &self)
{
    ...
    if (self && self->HasType() && self->GetType()->IsClass())
    {
             ...
             auto &[parent, function] : class_functions)
        {
            ...
            functions.push_back(std::move(reference));
        }
    }
    ...
    const std::optional<Field> &self) const
{
    ...
    for (auto &function : functions)
    {
        ...
        for (i = 0; i < parameter_count; ++i)
            if (Field::GetCastError(*this, function_type->GetParameter(i), arguments[i], error, false))
                break;
    ...
    const bool is_implicit)
{
    ...
    for (auto &[parent, function] : functions)
    {
        ...
        unsigned i;
        for (i = 0; i < parameter_count; ++i)
            if (Field::GetCastError(*this, function.Parameters[i], arguments[i], error, false))
                break;
        if (i < parameter_count)
            continue;
