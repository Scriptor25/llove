...
llove::Operator<1>::Ptr llove::Builder::FindOperator(const std::string &name, const Field &operand, bool suffix)
{
    ...
    if (const auto it = BIUnOperatorCallees.find(name); it != BIUnOperatorCallees.end())
        return std::make_unique<BIOperator<1>>(it->second, suffix);

    return nullptr;
}

llove::Operator<2>::Ptr llove::Builder::FindOperator(const std::string &name, const Field &left, const Field &right)
{
    auto lowest_error = ~0u;
    ...
    if (left.HasType() && left.GetType()->IsClass())
    {
        ...
        for (const auto functions = class_type->GetFunctions(class_type, name); auto &[parent, function] : functions)
        {
            ...
            if (Field::GetCastError(*this, Field(function.IsMutable, true, parent), left, error, true))
                continue;
            if (Field::GetCastError(*this, function.Parameters[0], right, error, false))
                continue;
            ...
            agg.Class = parent;
            agg.Name = function.Name;
            agg.Parameters = { { function.Parameters[0], {} } };
            agg.Result = function.Result;
    ...
    if (const auto it = BIBiOperatorCallees.find(name); it != BIBiOperatorCallees.end())
        return std::make_unique<BIOperator<2>>(it->second);
