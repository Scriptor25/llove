...
{
    if (const auto it = m_Strings.find(value); it != m_Strings.end())
        return it->second;
...
llvm::Value *llove::Builder::CreateAdd(llvm::Value *left, llvm::Value *right, const std::string &name)
{
...
llvm::Value *llove::Builder::CreateSub(llvm::Value *left, llvm::Value *right, const std::string &name)
{
    Assert(left != nullptr, "left must not be null");
    Assert(right != nullptr, "right must not be null");
...
llvm::CallInst *llove::Builder::CreateMemcpy(llvm::Value *dst, llvm::Value *src, llvm::Value *count)
{
    ...
    return m_LLVMBuilder.CreateIntrinsic(
        ...
        { dst->getType(), src->getType(), count->getType() },
        { dst, src, count });
}
