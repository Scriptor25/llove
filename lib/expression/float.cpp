...
try
{
    auto type = m_Type                      ? As<FloatType>(m_Type)
              : expect && expect->IsFloat() ? As<FloatType>(std::move(expect))
...
llove::StatementPtr llove::FloatExpression::Reflect(Context& context) const
try
{
    TypePtr type;
