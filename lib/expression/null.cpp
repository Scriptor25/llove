...
llove::NullExpression::NullExpression(
    Location loc,
...
llove::ValuePtr llove::NullExpression::GenVal(
    Builder& builder,
    TypePtr expect) const
try
{
    auto type = m_Type ? m_Type
              : expect && expect->IsPointer() ? As<PointerType>(std::move(expect))
...
llove::StatementPtr llove::NullExpression::Reflect(Context& context) const
try
{
    PointerType::Ptr type;
