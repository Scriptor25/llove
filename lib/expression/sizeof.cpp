...
llove::SizeofExpression::SizeofExpression(
    Location loc,
    TypePtr type)
...
llove::ValuePtr llove::SizeofExpression::GenVal(
    Builder& builder,
    const TypePtr expect) const
try
{
    const auto size = m_Type->SizeBits(builder);
    const auto size_type = expect && expect->IsInteger() ? As<IntegerType>(expect)
...
llove::StatementPtr llove::SizeofExpression::Reflect(Context& context) const
try
{
    TypePtr type;
