...
bool llove::Field::IsCastable(const Builder &builder, const Field &dst, const Field &src, const bool strict)
{
    if (dst.IsReference())
    {
        if (!src.IsReference())
...
{
    return m_IsReference == other.m_IsReference && m_IsMutable == other.m_IsMutable && m_Type == other.m_Type;
}
