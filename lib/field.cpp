...
llvm::Value* llove::Field::GenCast(
    Builder& builder,
    ValuePtr value,
    const bool unstable_ownership) const
{
    if (m_IsReference)
    {
        ...
        Assert(type == m_Type || (type->IsClass() && As<ClassType>(type)->InheritsFrom(m_Type)), "reference type mismatch");
        Assert(value->IsMutable() || !m_IsMutable, "reference mutability violation");
        return value->GetPointer();
    }

    Assert(unstable_ownership || !m_Type->IsClass() || !value->IsReference(), "implicitly removing ownership from lvalue");
...
{
    return std::string(m_IsMutable ? "M" : "") + std::string(m_IsReference ? "R" : "")
         + m_Type->Mangle();
}

bool llove::Field::TypeInfo(
    Builder& builder,
...
{
    return m_IsReference == other.m_IsReference && m_IsMutable == other.m_IsMutable
        && m_Type == other.m_Type;
}

void llove::Field::Reflect(
    Context& context,
    Field& field) const
{
