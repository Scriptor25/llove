...
llove::InstanceType::InstanceType(std::string name, std::vector<TypePtr> arguments)
    : m_Name(std::move(name)),
...
bool llove::InstanceType::TypeInfo(Builder & /* builder */, std::vector<llvm::Constant *> & /* dst */) const
{
    Error("template type '{}' does not have typeinfo", m_Name);
}
